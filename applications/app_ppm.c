	...
	for(;;) {
		...
		case PPM_CTRL_TYPE_DUTY_NOREV:
			...
			if (!(pulses_without_power < MIN_PULSES_WITHOUT_POWER && config.safe_start)) {
				float dutyval=utils_map(servo_val, -1.0, 1.0, -mcconf->l_max_duty, mcconf->l_max_duty);
				mc_interface_set_duty(dutyval);
				send_duty = true;
			}
			break;
		...
		case PPM_CTRL_TYPE_PID_NOREV:
			...
			if (!(pulses_without_power < MIN_PULSES_WITHOUT_POWER && config.safe_start)) {
				...
				//make servo_value spread-out from 200 to 500 erpm
				//(below activationServoVal, it may be due to slight error in trigger so we ignore it)
				/*
				float activationServoVal=0.05;
				if(abs(servo_val)>0.01){
					servo_val=servo_val/abs(servo_val)*utils_map(servo_val,activationServoVal,1, 200/config.pid_max_erpm, 1);
				}
				mc_interface_set_pid_speed(servo_val * config.pid_max_erpm);

				*/
				if(servo_val<0.33 &&servo_val>-0.33){
					// temporary duty mode: for 88~167 of servo value, it goes to const duty mode
					float dutyval=utils_map(servo_val, -0.33, 0.33, -0.045, 0.045);
					mc_interface_set_duty(dutyval);
				}else{
					float rpmval=utils_map(servo_val, -1,1,-config.pid_max_erpm,config.pid_max_erpm);
					//rpmval=rpmval;
					mc_interface_set_pid_speed(rpmval);
				}
